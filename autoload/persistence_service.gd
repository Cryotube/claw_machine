...
const HMAC_HASH := HashingContext.HASH_SHA256
...
func set_tutorial_completion(completed: bool, metadata: Dictionary = {}) -> void:
    ...
    var default_context: String = "complete" if completed else "reset"
    tutorial["context"] = String(metadata.get("context", default_context))
...
func _serialize_payload(payload: Dictionary) -> PackedByteArray:
    ...
    var plain: PackedByteArray = json.to_utf8_buffer()
    var salt: PackedByteArray = _crypto.generate_random_bytes(SALT_SIZE)
    var iv: PackedByteArray = _crypto.generate_random_bytes(IV_SIZE)
    var key: PackedByteArray = _derive_key(_get_master_passphrase(), salt, KEY_LENGTH)
    if key.size() != KEY_LENGTH:
        return PackedByteArray()
    var aes := AESContext.new()
    var start_result := aes.start(AESContext.MODE_CBC_ENCRYPT, key, iv)
    if start_result != OK:
        return PackedByteArray()
    ...
    var hmac: PackedByteArray = _crypto.hmac_digest(HMAC_HASH, key, body)
...
func _load_from_disk() -> void:
    ...
    if data.is_empty():
        var backup: PackedByteArray = FileAccess.get_file_as_bytes(_resolve_backup_path())
...
func _deserialize_payload(buffer: PackedByteArray) -> Dictionary:
    ...
    var body: PackedByteArray = buffer.slice(body_start, body_end)
    if body.size() <= SALT_SIZE + IV_SIZE:
        return {}
    var salt: PackedByteArray = body.slice(0, SALT_SIZE)
    var iv: PackedByteArray = body.slice(SALT_SIZE, SALT_SIZE + IV_SIZE)
    var cipher: PackedByteArray = body.slice(SALT_SIZE + IV_SIZE, body.size())
    var hmac: PackedByteArray = buffer.slice(body_end, buffer.size())
    var key: PackedByteArray = _derive_key(_get_master_passphrase(), salt, KEY_LENGTH)
    if key.size() != KEY_LENGTH:
        return {}
    var expected_hmac: PackedByteArray = _crypto.hmac_digest(HMAC_HASH, key, body)
    if expected_hmac != hmac:
        return {}
    var aes := AESContext.new()
    var start_result := aes.start(AESContext.MODE_CBC_DECRYPT, key, iv)
    if start_result != OK:
        return {}
...
func _derive_key(passphrase: String, salt: PackedByteArray, length: int) -> PackedByteArray:
    var password_bytes: PackedByteArray = passphrase.to_utf8_buffer()
    ...
    if length <= 0:
        return PackedByteArray()
    var block_count := int(ceil(float(length) / float(HMAC_BLOCK_BYTES)))
    var derived := PackedByteArray()
    for i in range(1, block_count + 1):
        var block := _pbkdf2_block(password_bytes, salt, i, PBKDF2_ITERATIONS)
        derived.append_array(block)
    derived.resize(length)
    return derived

func _pbkdf2_block(password: PackedByteArray, salt: PackedByteArray, block_index: int, iterations: int) -> PackedByteArray:
    var index_bytes := PackedByteArray([0, 0, 0, 0])
    var counter := block_index
    for i in range(3, -1, -1):
        index_bytes[i] = counter & 0xFF
        counter >>= 8
    var initial := salt.duplicate()
    initial.append_array(index_bytes)
    var u := _crypto.hmac_digest(HMAC_HASH, password, initial)
    var t := u.duplicate()
    for _i in range(1, max(iterations, 1)):
        u = _crypto.hmac_digest(HMAC_HASH, password, u)
        for j in range(t.size()):
            t[j] = t[j] ^ u[j]
    return t

func _get_master_passphrase() -> String:
    var app_name: String = String(ProjectSettings.get_setting("application/config/name", "claw_machine"))
    var device: String = ""
    if OS.has_feature("mobile"):
        device = OS.get_model_name()
    if device == "":
        device = String(OS.get_unique_id())
    if device == "":
        device = String(ProjectSettings.get_setting("application/config/description", "claw_machine"))
