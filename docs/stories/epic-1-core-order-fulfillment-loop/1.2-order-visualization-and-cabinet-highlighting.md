...
- [ ] **TDD & Performance Validation (AC: 1–4)** – GUT suite now passing via `./scripts/godot-cli.sh --headless --run res://tests/gut/test_runner.tscn`; profiler capture for highlight transitions still pending to record <16.67 ms frame times. [Source: architecture/test-strategy-and-standards.md#testing-philosophy][Source: architecture/performance-and-security-considerations.md#performance-budgets]
