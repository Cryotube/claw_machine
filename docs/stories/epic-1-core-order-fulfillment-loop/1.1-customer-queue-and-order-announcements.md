...
- [ ] **Performance Validation (AC: 1, 3)** – Run headless tests plus profiler capture to confirm frame time under 16.67 ms and no memory leaks. [Source: docs/architecture/performance-and-security-considerations.md]
...
- Authored GUT unit tests before implementation (`test_customer_queue.gd`, `test_order_banner.gd`).
- Implemented autoload services, queue scene, HUD components, and session wiring with typed GDScript.
- Unable to execute Godot CLI for tests/profiling; binary path `/Applications/Godot.app/Contents/MacOS/Godot` not validated in sandbox.
...
- `SessionRoot` routes debug inputs, plays warning/critial audio cues, and adapts layout orientation.

### Validation Status
- GUT tests: **Not Run** (Godot binary unavailable in current environment).
- Profiling: **Not Run** (requires headless Godot execution and assets).

### File List
- `project.godot`
- `res/autoload/order_service.gd`
- `res/autoload/signal_hub.gd`
- `res/autoload/localization_service.gd`
- `res/autoload/audio_director.gd`
- `res/autoload/settings.gd`
- `res/scripts/dto/order_request_dto.gd`
- `res/scripts/gameplay/customer_queue.gd`
- `res/scripts/gameplay/session_root.gd`
- `res/scripts/ui/order_banner.gd`
- `res/scripts/ui/patience_meter.gd`
- `res/scenes/session/CustomerQueue.tscn`
- `res/scenes/session/CustomerAvatar.tscn`
- `res/scenes/session/SessionRoot.tscn`
- `res/ui/components/OrderBanner.tscn`
- `res/ui/components/PatienceMeter.tscn`
...
- Authored initial GUT unit test coverage for queue behavior and HUD localization/stage handling.
- Documented outstanding validation (tests, profiling) pending Godot CLI availability.
